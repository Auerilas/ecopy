...
def beta_dispersion(X, groups, test="anova", scores=False, center="median", n_iter=99):
    ...
    if test == "anova":
        ...
        print(
            "{0:<10} {1:^5} {2:^8} {3:^8} {4:^8} {5:^8}".format(
                " ", "df", "SS", "MS", "F", "P(>F)"
            )
        )
        print(
            "{0:<10} {1:^5} {2:^8.4f} {3:^8.4f} {4:^8.4f} {5:^8.4f}".format(
                "Groups",
                int(obs[4]),
                obs[0],
                obs[2],
                obs[6],
                1 - f.cdf(obs[6], obs[4], obs[5]),
            )
        )
        print(
            "{0:<10} {1:^5} {2:^8.4f} {3:^8.4f}".format(
                "Residuals", int(obs[5]), obs[1], obs[3]
            )
        )
    if test == "permute":
...
def z_calc(U, groups, evals, center, iterations, permute=False):
    ...
    for i in range(n_groups):
        ...
        pos_euclidean = np.apply_along_axis(
            lambda t: np.sum((t - pos_cent) ** 2), 1, pos_Z
        )
        ...
        if np.sum(~pos) > 0:
            ...
            neg_euclidean = np.apply_along_axis(
                lambda t: np.sum((t - neg_cent) ** 2), 1, neg_Z
            )
    ...
    if permute:
        ...
        while k < iterations:
            ...
            res_perm_pos = res[
                np.ix_(np.random.choice(index1, len(index1), replace=False), pos)
            ]
