...
class rlq(object):
    ...
    def __init__(self, R, L, Q, ndim=2):
        ...
        if nr_t != nc_sp:
            msg = (
                "The number of matrix L columns must equal  the number of matrix Q rows"
            )
        if nr_e < nc_e:
        ...
        RLQmat = (
            envMat.T.dot(np.diag(row_w)).dot(Lmat).dot(np.diag(col_w)).dot(traitMat)
        )
        RLQmat = DataFrame(RLQmat, index=envNames)
        RLQmat.columns = traitNames
        axes, rowcoords, colcoords, components, self.evals = ordfunc(
            RLQmat, traitWeights, envWeights, ndim
        )
        ...
        self.traitVecs.columns = [
            "Trait Vector {0}".format(x) for x in range(1, ndim + 1)
        ]
        ...
        envScores.columns = [
            "Environment Component {0}".format(x) for x in range(1, ndim + 1)
        ]
        self.normedEnv = normalize(envScores, row_w)
        self.envVecs = DataFrame(np.real(components), index=envNames)
        self.envVecs.columns = [
            "Environmental Vector {0}".format(x) for x in range(1, ndim + 1)
        ]
    ...
    def summary(self):
        ...
        summDF = DataFrame(
            np.vstack((sds, props, cumprop)), index=["Std. Dev", "Prop Var", "Cum Var"]
        )
    ...
    def biplot(self, xax=1, yax=2):
        ...
        ax[0, 0].plot(
            self.normedTraits.iloc[:, xax - 1],
            self.normedTraits.iloc[:, yax - 1],
            "o",
            ms=0,
        )
        for i in range(self.normedTraits.shape[0]):
            ax[0, 0].text(
                self.normedTraits.iloc[i, xax - 1],
                self.normedTraits.iloc[i, yax - 1],
                self.normedTraits.index.values[i],
                ha="center",
                va="center",
            )
        ...
        ax[0, 1].plot(
            self.normedEnv.iloc[:, xax - 1], self.normedEnv.iloc[:, yax - 1], "o", ms=0
        )
        for i in range(self.normedEnv.shape[0]):
            ax[0, 1].text(
                self.normedEnv.iloc[i, xax - 1],
                self.normedEnv.iloc[i, yax - 1],
                self.normedEnv.index.values[i],
                ha="center",
                va="center",
            )
        ...
        for i in range(self.traitVecs.shape[0]):
            xlim = [
                np.min(self.traitVecs.iloc[:, xax - 1]) * 1.1,
                np.max(self.traitVecs.iloc[:, xax - 1]) * 1.1,
            ]
            ylim = [
                np.min(self.traitVecs.iloc[:, yax - 1]) * 1.1,
                np.max(self.traitVecs.iloc[:, yax - 1]) * 1.1,
            ]
            ax[1, 0].arrow(
                0,
                0,
                self.traitVecs.iloc[i, xax - 1],
                self.traitVecs.iloc[i, yax - 1],
                color="red",
                head_width=np.ptp(xlim) * 0.01,
            )
            ax[1, 0].text(
                self.traitVecs.iloc[i, xax - 1] * 1.1,
                self.traitVecs.iloc[i, yax - 1] * 1.1,
                self.traitVecs.index.values[i],
                color="r",
                ha="center",
                va="center",
            )
        ...
        for i in range(self.envVecs.shape[0]):
            xlim = [
                np.min(self.envVecs.iloc[:, xax - 1]) * 1.1,
                np.max(self.envVecs.iloc[:, xax - 1]) * 1.1,
            ]
            ylim = [
                np.min(self.envVecs.iloc[:, yax - 1]) * 1.1,
                np.max(self.envVecs.iloc[:, yax - 1]) * 1.1,
            ]
            ax[1, 1].arrow(
                0,
                0,
                self.envVecs.iloc[i, xax - 1],
                self.envVecs.iloc[i, yax - 1],
                color="blue",
                head_width=np.ptp(xlim) * 0.01,
            )
            ax[1, 1].text(
                self.envVecs.iloc[i, xax - 1] * 1.1,
                self.envVecs.iloc[i, yax - 1] * 1.1,
                self.envVecs.index.values[i],
                color="blue",
                ha="center",
                va="center",
            )
...
def dummyMat(matrix, rows, columns, row_w):
    ...
    for col in range(columns):
        if columnType[col] == "q":
            modMat = np.hstack(
                (modMat, wt_scale(matrix.iloc[:, col], row_w, bias=1).reshape(rows, 1))
            )
