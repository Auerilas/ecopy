...
class hillsmith(object):
    ...
    def __init__(self, mat, wt_r=None, ndim=2):
        def corfunc(x):
            ...
            if columnType[column] == "q":
                w = (
                    ...
                    * wt_r
                    * modMat[:, columnIndex == column].ravel()
                )
                return np.sum(w) ** 2
            else:
        ...
        for column in range(nc):
            if columnType[column] == "q":
                modMat = np.hstack(
                    (
                        modMat,
                        wt_scale(df.iloc[:, column], wt=wt_r, bias=1).reshape(nr, 1),
                    )
                )
        ...
        component_names = [
            "Component {0}".format(str(i + 1)) for i in range(components.shape[1])
        ]
    ...
    def biplot(self, invert=False, xax=1, yax=2, obsNames=True):
        ...
        for i in range(arrows.shape[0]):
            ...
            ax.text(
                arrows.iloc[i, xax - 1] * 1.2,
                arrows.iloc[i, yax - 1] * 1.2,
                arrows.index.values[i],
                color="red",
                ha="center",
                va="center",
            )
