...
class pcoa(object):
    ...
    def __init__(self, x, correction=None, siteNames=None):
        ...
        if correction is "2":
            ...
            d1 = 2.0 * D
            d2 = -0.5 * y
            d2 = -4.0 * (I - ones.dot(ones.T) / n).dot(d2).dot(I - ones.dot(ones.T) / n)
    ...
    def summary(self):
        ...
        sumTable = DataFrame(
            np.vstack((sds, props, cumSums)), index=["Std. Dev", "Prop.", "Cum. Prop."]
        )
    ...
    def biplot(
        ...
        if descriptors is not None:
            ...
            d2 = np.apply_along_axis(
                lambda x: (x - np.mean(x)) / np.std(x, ddof=1), 0, d2
            )
            if isinstance(descriptors, DataFrame):
            ...
            U2 = np.apply_along_axis(
                lambda x: (x - np.mean(x)) / np.std(x, ddof=1), 0, U2
            )
            S = (1.0 / (d2.shape[0] - 1)) * d2.T.dot(U2)
            dProj = np.sqrt(d2.shape[0] - 1) * S.dot(
                np.diag(self.evals[[xax - 1, yax - 2]] ** -0.5)
            )
        if not coords:
            f, ax = py.subplots()
            ...
            [
                ax.text(
                    x, y, s, color=siteCol, fontsize=siteSize, ha="center", va="center"
                )
                for x, y, s in zip(
                    self.U[:, xax - 1], self.U[:, yax - 1], self.siteLabs
                )
            ]
            if descriptors is not None:
                ...
                [
                    ...
                    for x, y, s in zip(dProj[:, 0], dProj[:, 1], dLabs)
                ]
