...
class anosim(object):
    ...
    def __init__(self, dist, factor1, factor2=None, nested=False, nperm=999):
        ...
        if factor2 is not None and not nested:
            ...
            self.p_val = [
                np.mean(self.r_perm1 > self.R_obs1),
        ...
        if factor2 is not None and nested:
            ...
            for i in range(nperm):
                ...
                for j in np.unique(comb2["group2"]):
                    gPerm = np.random.choice(
                        comb2["group1"][comb2["group2"] == j],
                        np.sum(comb2["group2"] == j),
                        replace=False,
                    )
                    permG.extend(gPerm)
                comb2["group1"] = permG
                ...
                for k in np.unique(comb2["group2"]):
                    ...
                    gpR.append(
                        oneWayANOSIM(withinMat, comb2["group1"][comb2["group2"] == k])
                    )
            ...
            uniqueSites = np.unique(comb)[np.unique(comb).argsort(order="group2")][
                "group1"
            ]
            collapseMat = np.zeros((len(uniqueSites), len(uniqueSites)))
            for i in range(len(uniqueSites)):
                for j in range(len(uniqueSites)):
                    collapseMat[i, j] = np.mean(
                        rankMat[comb["group1"] == uniqueSites[i], :][
                            :, comb["group1"] == uniqueSites[j]
                        ]
                    )
            ...
            for i in range(nperm):
                groupRand = np.random.choice(
                    collapseGroup, len(collapseGroup), replace=False
                )
    ...
    def summary(self):
        if self.R_obs2 is None:
            summ1 = "\nANOSIM\nObserved R = {0:.3}\np-value = {1:.3}\n{2} permutations".format(
                self.R_obs1, self.p_val, self.perm
            )
            return summ1
        else:
            summ1 = "\nANOSIM: Factor 1\nObserved R = {0:.3}\np-value = {1:.3}\n{2} permutations".format(
                self.R_obs1, self.p_val[0], self.perm
            )
            summ2 = "\nANOSIM: Factor 2\nObserved R = {0:.3}\np-value = {1:.3}\n{2} permutations".format(
                self.R_obs2, self.p_val[1], self.perm
            )
            return summ1 + "\n" + summ2
