...
class corner4(object):
            ...
            for j in range(Q.shape[1]):
                ...
                if R.iloc[:, i].dtype == "float" and Q.iloc[:, i].dtype == "float":
                    ...
                    while iteration < nperm:
                        ...
                        stat_perm[iteration] = QuantQuant(
                            R.iloc[:, i], Lperm, Q.iloc[:, j]
                        )
                        iteration += 1
        ...
        self.results = DataFrame(
            {
                "Comparison": compName,
                "Statistic": statType,
                "Observed Stat": np.round(obsStat, 2),
    ...
    def summary(self):
        if self.adj is None:
            ...
            return self.results[
                ["Comparison", "Statistic", "Observed Stat", "tail", "p-value"]
            ]
        if self.adj is not None:
            ...
            self.results["adjusted p-value"] = p_adjust(
                self.results["p-value"], self.adj
            )
            return self.results[
                [
                    "Comparison",
...
def QualQual(R, L, Q, n_env, n_trait):
    ...
    for i in range(n_env):
        for j in range(n_trait):
            crosstab[i, j] = np.argwhere(
                (inflMat[:, 0] == i) & (inflMat[:, 1] == j)
            ).shape[0]
...
def QualQuant(R, L, Q):
    ...
    btwn = inflMat.groupby("Ro")["Qo"].agg(
        {"mean": lambda x: len(inflMat) * (x.mean() - inflMat["Qo"].mean()) ** 2}
    )
    btwn = btwn["mean"].sum() / (len(btwn) - 1)
    within = inflMat.groupby("Ro")["Qo"].agg(
        {"mean": lambda x: np.sum((x - x.mean()) ** 2)}
    )
    ...
    for i in range(L2.shape[0]):
        for j in range(L2.shape[1]):
            if L2[i, j] != 0:
                ...
                Qo.extend([Q2[j]] * L2[i, j])
    inflMat = DataFrame({"Ro": Ro, "Qo": Qo})
    btwn = inflMat.groupby("Qo")["Ro"].agg(
        {"mean": lambda x: len(inflMat) * (x.mean() - inflMat["Qo"].mean()) ** 2}
    )
    btwn = btwn["mean"].sum() / (len(btwn) - 1)
    within = inflMat.groupby("Qo")["Ro"].agg(
        {"mean": lambda x: np.sum((x - x.mean()) ** 2)}
    )
    within = within["mean"].sum() / (len(inflMat) - len(within))
    return btwn / within
