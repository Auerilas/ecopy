...
class ca(object):
    ...
    def __init__(self, x, siteNames=None, spNames=None, scaling=1):
        ...
        if self.Trans:
            self.cumDesc_Sp = DataFrame(
                np.apply_along_axis(
                    lambda x: np.cumsum(x ** 2) / np.sum(x ** 2), 1, Uhat2
                )
            )
            self.cumDesc_Site = DataFrame(
                np.apply_along_axis(lambda x: np.cumsum(x ** 2) / np.sum(x ** 2), 1, U2)
            )
        else:
            self.cumDesc_Sp = DataFrame(
                np.apply_along_axis(lambda x: np.cumsum(x ** 2) / np.sum(x ** 2), 1, U2)
        ...
        self.cumDesc_Sp.columns = [
            "CA Axis " + str(x) for x in range(1, len(self.evals) + 1)
        ]
        self.cumDesc_Site.columns = [
            "CA Axis " + str(x) for x in range(1, len(self.evals) + 1)
        ]
    ...
    def summary(self):
        ...
        sumTable = DataFrame(
            np.vstack((sds, props, cumSums)), index=["Inertia", "Prop.", "Cum. Prop."]
        )
    ...
    def biplot(
        ...
        if showSite:
            ax.plot(
                self.siteScores.iloc[:, xax - 1],
                self.siteScores.iloc[:, yax - 1],
                "ko",
                ms=0,
            )
            [
                ax.text(
                    x, y, s, fontsize=siteSize, color=siteCol, ha="center", va="center"
                )
                for x, y, s in zip(
                    self.siteScores.iloc[:, xax - 1],
                    self.siteScores.iloc[:, yax - 1],
        ...
        if showSp:
            ax.plot(
                self.spScores.iloc[:, xax - 1],
                self.spScores.iloc[:, yax - 1],
                "k^",
                ms=0,
            )
            [
                ax.text(x, y, s, fontsize=spSize, color=spCol, ha="center", va="center")
                for x, y, s in zip(
                    self.spScores.iloc[:, xax - 1],
                    self.spScores.iloc[:, yax - 1],
                    self.spLabs,
            ...
            xmax = max(
                np.amax(self.siteScores.iloc[:, xax - 1]),
                np.amax(self.spScores.iloc[:, xax - 1]),
            )
            xmin = min(
                np.amin(self.siteScores.iloc[:, xax - 1]),
                np.amin(self.spScores.iloc[:, xax - 1]),
            )
            ymax = max(
                np.amax(self.siteScores.iloc[:, yax - 1]),
                np.amax(self.spScores.iloc[:, yax - 1]),
            )
            ymin = min(
                np.min(self.siteScores.iloc[:, yax - 1]),
                np.min(self.spScores.iloc[:, yax - 1]),
            )
