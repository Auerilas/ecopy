...
class ccor(object):
    ...
    def __init__(
        ...
        S1 = D1.T.dot(D1) * 1.0 / df
        S2 = D2.T.dot(D2) * 1.0 / df
        S12 = D1.T.dot(D2) * 1.0 / df
    ...
    def biplot(self, matrix=1, xax=1, yax=2):
        ...
        for i in range(loadings.shape[0]):
            ax.arrow(
                0,
                0,
                loadings.iloc[i, xplot],
                loadings.iloc[i, yplot],
                color="b",
                head_width=0.1,
            )
            ax.text(
                loadings.iloc[i, xplot] * 1.2,
                loadings.iloc[i, yplot] * 1.2,
                varNames[i],
                color="b",
                ha="center",
                va="center",
