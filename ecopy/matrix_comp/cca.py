...
class cca(object):
    ...
    def __init__(
        self, Y, X, varNames_y=None, varNames_x=None, rowNames=None, scaling=1
    ):
        ...
        B = (
            np.linalg.pinv(x_scale.T.dot(W).dot(x_scale))
            .dot(x_scale.T)
            .dot(W ** 0.5)
            .dot(Q_mat)
        )
        ...
        self.resid = np.array(
            DataFrame(Q_mat - Yhat, columns=varNames_y, index=rowNames)
        )
        ...
        if scaling == 1:
            ...
            self.siteFitted = DataFrame(
                Wrow.dot(Yhat).dot(self.U), columns=CAlist, index=rowNames
            )
            ...
            self.varScores = DataFrame(
                x_scale.T.dot(Wrow).dot(scaled_Z).dot(np.diag(self.evals ** 0.5)),
                columns=CAlist,
                index=varNames_x,
            )
        if scaling == 2:
            ...
            self.siteFitted = DataFrame(
                Wrow.dot(Yhat).dot(self.U).dot(np.diag(self.evals ** -0.5)),
                columns=CAlist,
                index=rowNames,
            )
            mu_z = self.r_w.dot(self.siteFitted)
            ...
            scaled_Z = D_z.dot(scale_z_mat)
            self.varScores = DataFrame(
                x_scale.T.dot(Wrow).dot(scaled_Z), columns=CAlist, index=varNames_x
            )
    ...
    def summary(self):
        ...
        data = np.vstack(
            (np.round(self.evals, 3), np.round(self.evals / self.evals.sum(), 3))
        )
        ...
        data2 = np.vstack(
            (
                np.round(self.res_evals, 3),
                np.round(self.res_evals / self.res_evals.sum(), 3),
            )
        )
        SumTable2 = DataFrame(
            data2, index=["Variance", "Prop. Variance"], columns=names2
        )
    ...
    def triplot(self, xax=1, yax=2):
        ...
        for i in range(self.spScores.shape[0]):
            ...
            ax.text(
                self.spScores.iloc[i, xplot],
                self.spScores.iloc[i, yplot],
                self.spScores.index.values[i],
                color="r",
                ha="center",
                va="center",
            )
        for i in range(self.siteScores.shape[0]):
            ax.plot(
                self.siteScores.iloc[i, xplot], self.siteScores.iloc[i, yplot], ms=0
            )
            ax.text(
                self.siteScores.iloc[i, xplot],
                self.siteScores.iloc[i, yplot],
                self.siteScores.index.values[i],
                color="k",
                ha="center",
                va="center",
            )
        for i in range(self.varScores.shape[0]):
            ax.arrow(
                0,
                0,
                self.varScores.iloc[i, xplot],
                self.varScores.iloc[i, yplot],
                color="b",
                head_width=0.1,
            )
            ax.text(
                self.varScores.iloc[i, xplot] * 1.2,
                self.varScores.iloc[i, yplot] * 1.2,
                self.varScores.index.values[i],
                color="b",
                ha="center",
                va="center",
            )
        xmins = (
            self.spScores.iloc[:, xplot].min(),
            self.siteScores.iloc[:, xplot].min(),
            self.varScores.iloc[:, xplot].min() * 1.2,
        )
        xmax = (
            self.spScores.iloc[:, xplot].max(),
            self.siteScores.iloc[:, xplot].max(),
            self.varScores.iloc[:, xplot].max() * 1.2,
        )
        ymins = (
            self.spScores.iloc[:, yplot].min(),
            self.siteScores.iloc[:, yplot].min(),
            self.varScores.iloc[:, yplot].min() * 1.2,
        )
        ymax = (
            self.spScores.iloc[:, yplot].max(),
            self.siteScores.iloc[:, yplot].max(),
            self.varScores.iloc[:, yplot].max() * 1.2,
        )
