...
class rda(object):
    ...
    def __init__(
        ...
        B = (
            np.linalg.pinv(self.x_mat.T.dot(self.x_mat))
            .dot(self.x_mat.T)
            .dot(self.y_mat)
        )
        ...
        for i in range(self.x_mat.shape[1]):
            for j in range(len(RDA_evals)):
                self.corr[i, j] = np.corrcoef(
                    self.x_mat[:, i], self.linSites.iloc[:, j]
                )[0, 1]
        ...
        self.m = len(B)
        self.R2a = 1.0 - (1.0 - self.R2) * ((self.n - 1.0) / (self.n - self.m - 1.0))
        ...
        self.resid_spScores = DataFrame(
            residU.dot(np.diag(self.resid_evals ** 0.5)), index=varNames_y
        )
        self.resid_siteScores = DataFrame(
            residuals.dot(residU).dot(np.diag(self.resid_evals ** 0.5)), index=rowNames
        )
        PC_names = [
            "PC Axis {0}".format(x) for x in range(1, len(self.resid_evals) + 1)
        ]
        ...
        self.imp = DataFrame(
            np.vstack((sds, props, cums)), index=["Std Dev", "Prop Var", "Cum Var"]
        )
        self.imp.columns = RDA_names

    def summary(self, n=10):
        print(
            "\nTotal Variance = {0:.3}".format(
                np.sum(self.RDA_evals) + np.sum(self.resid_evals)
            )
        )
        ...
        for i in range(self.corr.shape[0]):
            if self.pTypes[i] == "q":
                ax.arrow(
                    0,
                    0,
                    self.corr.iloc[i, xplot],
                    self.corr.iloc[i, yplot],
                    color="b",
                    head_width=0.1,
                )
                ax.text(
                    self.corr.iloc[i, xplot] * 1.2,
                    self.corr.iloc[i, yplot] * 1.2,
                    self.corr.index.values[i],
                    color="b",
                    ha="center",
                    va="center",
                )
