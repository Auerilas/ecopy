...
class MDS(object):
        ...
        if transform is "absolute":
            ...
            for i in range(ntry):
                if i == 0:
                    ...
                    Z = np.random.rand(distmat.shape[0] * naxes).reshape(
                        distmat.shape[0], naxes
                    )
                stress1 = 1
                k = 0
                e = 1e4
        ...
        if descriptors is not None:
            ...
            if descripNames is None:
                if isinstance(descriptors, DataFrame):
                    ...
                    descripNames = [
                        "Species {0}".format(i) for i in range(descriptors.shape[1])
                    ]
        if not coords:
            f, ax = plt.subplots()
            ...
            if siteNames:
                ...
                [
                    ...
                    for x, y, s in zip(
                        self.scores[:, xax - 1], self.scores[:, yax - 1], self.siteLabs
                    )
                ]
            else:
            ...
            if descriptors is not None:
                [
                    ax.text(
                        x, y, s, color=spCol, fontsize=spSize, ha="center", va="center"
                    )
                    for x, y, s in zip(dScores[:, 0], dScores[:, 1], descripNames)
                ]
    ...
    def shepard(self):
        ...
        if self.transform is "monotone":
            ...
            R2 = (
                1
                - ((dHats - monoPreds.prediction) ** 2).sum()
                / ((dHats - dObs) ** 2).sum()
            )
            o = dObs.argsort()
    ...
    def correlationPlots(self, site=None):
        ...
        if site is None:
            ...
            [
                ...
                for x, y, s in zip(
                    dObs[site, :], dFit[site, :], range(len(dFit[site, :]))
                )
            ]
...
def eucD(X):
    ...
    one = np.ones((n, 1))
    D = (
        np.sum(X ** 2, 1).reshape(n, 1).dot(one.T)
        + one.dot(np.sum(X ** 2, 1).reshape(1, n))
        - 2 * X.dot(X.T)
    )
    D = np.round(D, 10)
    D = np.sqrt(D)
...
def absMDS(distmat, Z, weights, Vp):
    ...
    dXu = eucD(Xu)
    stress = np.sqrt(
        np.tril(weights * (distmat - dXu) ** 2).sum() / np.tril(dXu ** 2).sum()
    )
    return stress, Xu
...
def ratioMDS(distmat, b, Z, weights, Vp):
    ...
    dXu = eucD(Xu)
    stress = np.sqrt(
        np.tril(weights * (dHat - dXu) ** 2).sum() / np.tril(dXu ** 2).sum()
    )
...
def linMDS(distmat, a, b, Z):
    ...
    a = (np.tril(weights * dXu).sum() - b * np.tril(weights * distmat).sum()) / np.tril(
        weights
    ).sum()
    b = (
        np.tril(weights * distmat * dXu).sum() - a * np.tril(weights * distmat).sum()
    ) / np.tril(weights * distmat ** 2).sum()
...
def nMDS(distmat, Z, weights, Vp):
    ...
    stress = np.sqrt(
        np.tril((weights * (dZ - dhat) ** 2)).sum() / np.tril(weights * dZ ** 2).sum()
    )
    bZ = Bcalc(weights, dhat, dZ)
    Xu = Vp.dot(bZ).dot(Z)
    return stress, Xu
